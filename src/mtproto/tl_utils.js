...
import { bigint, gzipUncompress, bytesToArrayBuffer, intToUint, uintToInt,bigStringInt } from "./bin_utils.js";
...
function TLDeserialization(buffer, options) {
  ...
  this.buffer = buffer
  this.intView = new Int32Array(this.buffer.buffer)
  this.byteView = new Uint8Array(this.buffer)
...
TLDeserialization.prototype.fetchObject = function (type, field) {
  ...
  if (type.charAt(0) == '%') {
    ...
    if (constructorCmp == 0x3072cfa1) { // Gzip packed
      ...
      var uncompressed = gzipUncompress(compressed)
      var buffer = bytesToArrayBuffer(uncompressed)
      var newDeserializer = (new TLDeserialization(new Uint8Array(buffer)))
    ...
    for (i = 0; i < schema.methods.length; i++) {
      if (schema.methods[i].id == constructorCmp) {
        constructorData = schema.methods[i]
      }
    }
    ...
    if (!constructorData && this.mtproto) {
      ...
      for (i = 0; i < schemaFallback.methods.length; i++) {
        if (schemaFallback.methods[i].id == constructorCmp) {
          constructorData = schemaFallback.methods[i]
  ...
  predicate = constructorData.predicate || constructorData.method
...
export { TLSerialization, TLDeserialization };
